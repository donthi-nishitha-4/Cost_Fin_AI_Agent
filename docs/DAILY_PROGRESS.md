...
2. **LangSmith Throttling**: Injected `max_concurrency=2` into the LangSmith evaluator script.
...
3. **Dataset History Preservation**: Refactored `generate_golden_dataset.py` to use `update_example()` instead of `delete_dataset()`.
