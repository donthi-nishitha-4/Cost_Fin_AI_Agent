...
def get_intent(query: str) -> str:
    query = query.lower()
    if "breakdown" in query: return "cost_breakdown"
    if "labor cost" in query or "material cost" in query or "equipment cost" in query: return "cost_lookup"
    if "budget" in query and "over" in query or "under" in query: return "budget_status"
    if "compare" in query: return "comparison"
    if "variance" in query: return "variance"
    if "risk" in query: return "overrun_risk"
    if "summary" in query: return "financial_summary"
    if "which subsystem" in query or "find" in query or "percentage of planned cost" in query: return "aggregation"
    return "out_of_scope"
...
def main():
    ...
    results = evaluate(
        ...
        experiment_prefix="V4_QueryAware_Eval",
    )
    print("\nEvaluation complete!")
